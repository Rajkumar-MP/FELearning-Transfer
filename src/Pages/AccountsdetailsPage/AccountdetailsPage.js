...
export class AccountdetailsPage extends LitElement {
  ...
  async firstUpdated() {
    const data = await FeServices.getRequest({url:'/accountinfo/925548553975232'});
  ...
  render() {
    ...
    ${this.data.map((item) => html`<fe-card title=${item.accountDetails.type} content=${item.accountDetails.balance}></fe-card>`)}
